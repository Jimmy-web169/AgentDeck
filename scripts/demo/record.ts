...
//
//   node scripts/demo/make-fixture.ts && npx vite build
//   AGENTDECK_CONFIG_DIR=tmp/demo-root AGENTDECK_PORT=47861 AGENTDECK_WEB_PORT=47862 node server/index.ts &
//   node scripts/demo/record.ts --base http://localhost:47861
//
// Usage:
//   node scripts/demo/record.ts [--base http://localhost:47861] [--release v2.0] [--out <dir>] [--fixture tmp/demo-root]
...
if (A.help) {
  console.log(
    'usage: node scripts/demo/record.ts [--base http://localhost:47861] [--release v2.0] [--out <dir>] [--fixture tmp/demo-root] [--theme graphite] [--w 1440] [--h 900] [--gif-width 1280] [--fps 15] [--browser exe] [--keep-frames] [--no-mp4]'
  )
  process.exit(0)
}
...
const THEME = argText(A.theme) || 'midnight'
...
class Tour {
  ...
  async rectOf(expr: string) {
    return this.cdp.eval<{ x: number; y: number; w: number; h: number } | null>(
      `(() => { const el = (${expr}); if (!el) return null; el.scrollIntoView({ block: 'nearest' }); const r = el.getBoundingClientRect(); return { x: r.left + r.width / 2, y: r.top + r.height / 2, w: r.width, h: r.height } })()`
    )
  }
  async moveTo(x: number, y: number, settle = 480) {
    await this.cdp.eval(
      `(() => { const c = window.__demoCursor; if (c) c.style.transform = 'translate(' + ${Math.round(x)} + 'px,' + ${Math.round(y)} + 'px)'; return true })()`
    )
    await this.cdp.send('Input.dispatchMouseEvent', { type: 'mouseMoved', x, y })
    await sleep(settle)
  }
  async click(expr: string, { settle = 480, after = 600 } = {}) {
  ...
  async type(text: string, perChar = 110) {
    for (const ch of text) {
      await this.cdp.send('Input.insertText', { text: ch })
      await sleep(perChar)
    }
  }
  async key(key: string, code: string) {
  ...
  async wheel(x: number, y: number, deltaY: number, steps = 4, gap = 260) {
  ...
  async wait(expr: string, timeout = 8000) {
...
async function tour(t: Tour, fx: Fixture, cdp: Cdp) {
  ...
  const byText = (tag: string, text: string) =>
    `[...document.querySelectorAll(${JSON.stringify(tag)})].find((b) => b.offsetParent !== null && (b.textContent || '').trim() === ${JSON.stringify(text)})`
  ...
  if (fx.claudeStar) {
    ...
    await t.click(`[...document.querySelectorAll('[title]')].find((el) => (el.getAttribute('title') || '').startsWith(${JSON.stringify(tabTitle)}))`, {
      after: 1200,
    })
    await t.wait(`/\\d+ prompts?/.test(${T})`, 8000)
    ...
    if (await t.wait(`!!(${thread})`, 4000)) {
      ...
      await cdp.eval(
        `(() => { const b = [...document.querySelectorAll('button')].find((x) => /hide thread/.test(x.textContent || '')); const block = b && b.closest('div.my-2'); const call = block && block.previousElementSibling; (call || block || b)?.scrollIntoView({ block: 'start', behavior: 'smooth' }); return true })()`
      )
      await sleep(1600)
    }
  }
  ...
  if (fx.agyStar) {
    ...
    await t.click(
      `[...document.querySelectorAll('[title]')].filter((el) => (el.getAttribute('title') || '').startsWith(${JSON.stringify(agyTab)})).pop() || null`,
      { after: 1200 }
    )
    await t.wait(`/\\d+ prompts?/.test(${T})`, 8000)
    await sleep(1200)
  ...
  if (fx.claudeStar) {
    ...
    await t.click(`[...document.querySelectorAll('button')].find((x) => x.offsetParent !== null && /^✦?\\s*Ask /.test((x.textContent || '').trim()))`, {
      after: 900,
    })
...
async function main() {
  ...
  const frames: { file: string; ts: number }[] = [] // { file, ts }
  ...
  const mb = (f: fs.PathLike) => (fs.statSync(f).size / 1048576).toFixed(1)
  let r = spawnSync(
    'ffmpeg',
    [
      '-y',
      ...
      list,
      '-vf',
      `fps=30,scale=${W}:-2:flags=lanczos,format=yuv420p`,
      '-c:v',
      'libx264',
      '-preset',
      'slow',
  ...
  const dither = argText(A.dither) || 'bayer:bayer_scale=5' // 'none' is crisper on flat UI, bayer is safer on gradients; both stay small
  r = spawnSync(
    'ffmpeg',
    [
      '-y',
      ...
      mp4,
      '-vf',
      `fps=${FPS},scale=${GIF_W}:-1:flags=lanczos,split[a][b];[a]palettegen=max_colors=256:stats_mode=diff[p];[b][p]paletteuse=dither=${dither}:diff_mode=rectangle`,
      '-loop',
      '0',
      gif,
    ],
