...
const run = (bin, args, timeout = 15_000) => {
  ...
  return { ok: r.status === 0, out: `${r.stdout || ''}${r.stderr || ''}`.trim(), missing: r.error?.code === 'ENOENT' || /not recognized|not found|No such file/i.test(r.stderr || '') && r.status !== 0 }
}
