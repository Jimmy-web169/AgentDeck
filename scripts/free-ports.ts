...
function canBind(port: number | undefined) {
  return new Promise((resolve) => {
    const srv = createServer()
    srv.once('error', (e: NodeJS.ErrnoException) => resolve(e.code || String(e)))
...
for (const port of ports) {
  ...
  for (const pid of pids) {
    try {
      ...
      console.log(`could not free :${port} (pid ${pid}): ${e instanceof Error ? e.message : String(e)}`)
    }
  }
}
