...
// literal in api.js — the binary names handed to findOnPath() and the resumeArgs
...
//
// Usage: node scripts/check-providers.mjs [--skills] [--json]
...
import { findOnPath, findTmux, findTtyd } from '../server/shared/terminal.js'
...
const strings = (s) => [...(s || '').matchAll(/(['"`])((?:\\.|(?!\1).)*)\1/g)].map((m) => m[2])
...
function providerIds() {
  ...
  return fs
    ...
    .filter((d) => d.isDirectory() && fs.existsSync(path.join(dir, d.name, 'api.js')))
    .map((d) => d.name)
    .sort()
}

// TERMINAL_CONFIG is a module-private literal in api.js (importing the module
...
function readTerminalConfig(id) {
  const file = path.join(ROOT, 'server', 'providers', id, 'api.js')
  ...
  const ra = body.match(/resumeArgs\s*:\s*\(\s*(\w+)\s*\)\s*=>\s*\[([^\]]*)\]/)
...
const versionOf = (out) => (out.match(/\d+\.\d+(?:\.\d+)?(?:[-+][\w.-]+)?/) || [null])[0]

// `--resume` / `-p` must appear as an option token in the help text
const hasFlag = (help, flag) => new RegExp(`(^|[\\s,\\[(|])${esc(flag)}(?=$|[\\s,=\\]\\[<>|).])`, 'm').test(help)
// `resume` must be listed as a subcommand: a line that starts with the bare word
const hasSubcommand = (help, sub) => new RegExp(`^\\s{1,8}${esc(sub)}(?:\\s{2,}|,|\\s*$|\\s+[\\[<(])`, 'm').test(help)

function checkProvider(cfg) {
  const row = { kind: 'provider', name: cfg.id, binary: null, version: null, checks: [], fatal: false, note: cfg.envKey ? `env ${cfg.envKey}` : '' }
...
function checkTmux() {
  const row = { kind: 'plumbing', name: IS_WIN ? 'tmux (psmux)' : 'tmux', binary: findTmux(), version: null, checks: [], fatal: false, note: '' }
  if (!row.binary) {
...
function checkTtyd() {
  ...
  const row = { kind: 'plumbing', name: 'ttyd', binary: bin, version: null, checks: [], fatal: false, note: IS_WIN ? 'not used on win32 (webterm.js serves the terminal)' : '' }
  if (!bin) {
...
async function checkNodePty() {
  ...
  const row = { kind: 'plumbing', name: 'webterm (node-pty)', binary: `${pkg}-${process.platform}-${process.arch}`, version: null, checks: [], fatal: false, note: IS_WIN ? '' : 'not used off win32 (ttyd serves the terminal)' }
  try {
  ...
  try {
    ...
    row.checks.push({ what: 'import', ok: false, detail: String(e.message || e).split('\n')[0] })
  }
  row.fatal = IS_WIN && row.checks.some((c) => !c.ok)
  return row
}

// `npx -y skills add <ref> -a <agent> [-g] -y` (server/shared/skills.js)
function checkSkills() {
  const row = { kind: 'plumbing', name: 'skills (npx)', binary: 'npx skills', version: null, checks: [], fatal: false, note: '' }
...
if (!WANT_JSON) {
  console.log('')
  console.log(failed.length ? `check-providers: FAIL (${failed.map((r) => r.name).join(', ')})` : `check-providers: ok (${process.platform}-${process.arch}, node ${process.version})`)
}
