...
//
// Usage: node scripts/check-privacy.mjs [--tracked-only] [--quiet]
...
const BINARY_EXT = new Set(['.png', '.jpg', '.jpeg', '.gif', '.webp', '.ico', '.mp4', '.webm', '.mov', '.woff', '.woff2', '.ttf', '.otf', '.eot', '.pdf', '.zip', '.gz', '.tgz', '.7z', '.jar', '.exe', '.dll', '.so', '.dylib', '.wasm', '.node'])

// user-name placeholders that documentation legitimately uses in example paths
const PLACEHOLDERS = new Set(['<name>', '<you>', '<user>', '<username>', '<your-name>', '<yourname>', '<USER>', '<USERNAME>', 'you', 'yourname', 'your-name', 'username', 'USERNAME', '%USERNAME%', '$USER', '${USER}', '$env:USERNAME', 'me', '...', '…'])
...
const isBinary = (file, buf) => BINARY_EXT.has(path.extname(file).toLowerCase()) || buf.subarray(0, 8000).includes(0)

function allowed(match, name, rules) {
...
function scanFile(file, text, rules) {
  const findings = []
...
const licenseLine = licenseFile ? (fs.readFileSync(path.join(ROOT, licenseFile), 'utf8').split(/\r?\n/).find((l) => l.trim()) || '').trim() : null
