// Original test group: activity. Assertions retained during module-path migration.
import test from 'node:test'
import assert from 'node:assert/strict'
import { bucketActivity, dayKey, totalTokens } from '../../../server/shared/activity.ts'

// a fixed local "now": Monday 2026-09-07 12:00
const NOW = new Date(2026, 8, 7, 12, 0, 0).getTime()
const at = (daysAgo: number, hour = 10) => new Date(2026, 8, 7 - daysAgo, hour, 0, 0).toISOString()
const sess = (daysAgo: number, extra = {}) => ({
  id: `s${daysAgo}-${hour(extra)}`,
  slug: 'proj-a',
  cwd: '/w/a',
  lastTs: at(daysAgo, hour(extra)),
  userTurns: 2,
  toolCalls: 5,
  tokens: { input: 100, output: 50, cacheRead: 25, cacheCreate: 0 },
  models: ['m1'],
  ...extra,
})
const hour = (x: { hour?: number }) => x.hour ?? 10
...
test('bucketActivity: projects rank by sessions then tokens; models count sessions', () => {
  const a = bucketActivity(
    [
      sess(0),
      sess(1),
      sess(2, { slug: 'proj-b', cwd: '/w/b', tokens: { input: 9000 }, models: ['m2'] }),
      sess(3, { slug: 'proj-c', cwd: '/w/c', models: ['m1', 'm2'] }),
    ],
    { days: 30, now: NOW }
  )
  assert.deepEqual(
...
test('bucketActivity: personal shape — durations, prompt buckets, weekly, compare, rhythm, neglected', () => {
  const withDur = (daysAgo: number, hour: number | undefined, minutes: number, extra = {}) => {
  ...
  assert.deepEqual(
    a.sessionsDetail.durationBuckets.map((b) => b.n),
    [1, 1, 1, 1, 1]
  )
  assert.deepEqual(
    a.sessionsDetail.promptBuckets.map((b) => b.n),
    [1, 3, 0, 0, 1]
  )
  assert.equal(a.weekly.at(-1)?.weekStart, '2026-09-07') // this Monday
  assert.equal(
    a.weekly.reduce((n, w) => n + w.sessions, 0),
    5
  )
...
test('bucketActivity: tolerates junk and empty input', () => {
  const a = bucketActivity([{ id: 'x' }, { id: 'y', lastTs: 'not a date' }, null] as unknown as Parameters<typeof bucketActivity>[0], { days: 3, now: NOW })
