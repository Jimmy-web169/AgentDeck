...
test('dispatch: routes by "METHOD /path", 404 for unknown, errors keep their status', async () => {
  ...
  const boom = await dispatch('GET', '/api/boom', q)
  assert.equal(boom.status, 500)
  assert.deepEqual(boom.body, { error: 'unexpected' })
  assert.ok(boom.cause instanceof Error && boom.cause.message === 'unexpected', 'an unexpected failure carries its cause for the host log')
