...
// Original test group: handoff. Assertions retained during module-path migration.
import test from 'node:test'
import assert from 'node:assert/strict'
import fs from 'node:fs'
import path from 'node:path'
import { composeBrief, writeBrief, seedPrompt } from '../../../server/shared/handoff.ts'
...
test('composeBrief: request, where, how to work, docs first, current content fenced', () => {
  const md = composeBrief({
    need: 'Run prettier after every Edit.',
    providerLabel: 'Codex',
    kind: 'hooks (config.toml)',
    filePath: '/home/demo/.codex/config.toml',
    content: '[hooks]\n',
    docs: 'https://developers.openai.com/codex/hooks',
    cwd: '/home/demo/code/orbit-api',
  })
...
test('composeBrief: docs index and the provider\x27s building blocks are listed for the interview', () => {
  const md = composeBrief({
    need: 'set this project up for me',
    providerLabel: 'Codex',
    cwd: '/home/demo/code/orbit-api',
    docsIndex: 'https://developers.openai.com/codex',
    kinds: [
      { name: 'skills', docs: 'https://developers.openai.com/codex/skills' },
      { name: 'MCP servers', docs: null },
    ],
  })
...
test('writeBrief stores under <configDir>/.agentdeck/handoffs, keeps the newest 40, and the seed prompt names the file', async () => {
  const dir = temporaryDirectory('agentdeck-handoff-')
  await withConfigDir(dir, () => {
    const files = []
    for (let i = 0; i < 43; i++) files.push(writeBrief(`brief ${i}\r\n`, { key: `k${i}` }))
    assert.ok(files.every((f) => f.startsWith(path.join(dir, '.agentdeck', 'handoffs'))))
    assert.equal(fs.readFileSync(files[42], 'utf8'), 'brief 42\n', 'CRLF normalised')
    const left = fs.readdirSync(path.join(dir, '.agentdeck', 'handoffs')).filter((f) => f.endsWith('.md'))
