// Windows regression coverage: Input validation for the Install-a-skill UI (`parseSkillsAdd`): accepts real skill refs and full `npx skills add` commands, strips user agent flags when the provider pins its own, and **rejects non-skill input** (the backend spawns a real process from this string).
// Original test group: windows/skills-parse. Assertions retained during module-path migration.
...
import { parseSkillsAdd, buildSkillsArgv } from '../../../server/shared/skills.ts'
