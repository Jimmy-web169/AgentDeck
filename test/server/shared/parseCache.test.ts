...
// Original test group: parse-cache. Assertions retained during module-path migration.
...
const { cachedRecords, cachedDerived, fingerprintOf, etagOf, invalidate } = await import('../../../server/shared/parseCache.ts')

const dir = temporaryDirectory('parse-cache-')
...
const countingParse = (counter: { n: number }) => (file: fs.PathOrFileDescriptor) => {
  counter.n++
