interface ProbeRecord {
  timestamp?: unknown
  type?: unknown
  payload?: { info?: { total_token_usage?: Record<string, unknown> }; [key: string]: unknown }
  [key: string]: unknown
}
import { temporaryDirectory, withConfigDir } from '../../helpers/tmpConfigDir.ts'
// Original test group: format-probe. Assertions retained during module-path migration.
...
import {
  loadProbeSpec as nullableSpec,
  expandGlob,
  sampleRecords,
  observe,
  compare,
  valuesAt,
  runProbe as nullableRun,
  acceptProbe as nullableAccept,
  probeStatus as nullableStatus,
} from '../../../server/shared/formatProbe.ts'
...
const tmp = () => temporaryDirectory('agentdeck-probe-')

const copy = (from: string, to: string) => {
  fs.mkdirSync(path.dirname(to), { recursive: true })
  fs.copyFileSync(from, to)
}
const lines = (file: fs.PathOrFileDescriptor): ProbeRecord[] =>
  fs
    .readFileSync(file, 'utf8')
    .split('\n')
    .filter(Boolean)
    .map((l) => JSON.parse(l))
const writeLines = (file: string, recs: unknown[]) => fs.writeFileSync(file, recs.map((r) => JSON.stringify(r)).join('\n') + '\n')

// homes laid out per the descriptors' sample globs
function claudeHome() {
  const home = tmp()
  copy(
    path.join(FX, 'claude', 'session-00fb8673.jsonl'),
    path.join(home, 'projects', 'C--home-demo-code-orbit-api', '00fb8673-8b42-4835-a84f-3e32248c1e89.jsonl')
  )
  return home
}
function codexHome() {
  const home = tmp()
  copy(
    path.join(FX, 'codex', 'session-3d5465c2.jsonl'),
    path.join(home, 'sessions', '2026', '09', '01', 'rollout-2026-09-01T06-20-00-3d5465c2-6ffd-76db-9962-51657f9592e0.jsonl')
  )
  return home
}
function agyHome() {
  const home = tmp()
  copy(
    path.join(FX, 'antigravity', 'session-7d1c4e2a.jsonl'),
    path.join(home, 'brain', '7d1c4e2a-9b3f-4c5d-8e6f-0a1b2c3d4e5f', '.system_generated', 'logs', 'transcript_full.jsonl')
  )
  return home
}
...
test('a missing required key, an unknown enum value and a type change are drift; a new optional key is only changed', async () => {
  ...
  await withConfigDir(dir, async () => {
    ...
    assert.ok(fs.existsSync(path.join(dir, '.agentdeck', 'probe', 'codex.json')), 'baseline persisted under its state owner')

    // the vendor adds an optional key → changed, not drift
    const recs = lines(file)
    writeLines(
      file,
      recs.map((r) => ({ ...r, trace_id: 'abc' }))
    )
    ...
    assert.ok(
      r.details.some((d) => d.level === 'drift' && /weird_new_record/.test(d.msg)),
    ...
    assert.equal(r.status, 'drift')
    assert.ok(
      r.details.some((d) => /required key "timestamp"/.test(d.msg)),
      JSON.stringify(r.details)
    )
    ...
    writeLines(
      ...
      recs.map((r) =>
        r.payload?.info?.total_token_usage
          ? { ...r, payload: { ...r.payload, info: { ...r.payload.info, total_token_usage: { ...r.payload.info.total_token_usage, total_tokens: '12' } } } }
          : r
      )
    )
    r = runProbe('codex', root)
    assert.ok(
      r.details.some((d) => /total_tokens is string, expected number/.test(d.msg)),
      JSON.stringify(r.details)
    )
...
test('probeStatus survives a restart from the store, and an empty root is "empty"', async () => {
  ...
  await withConfigDir(dir, async () => {
    ...
    const store = JSON.parse(fs.readFileSync(path.join(dir, '.agentdeck', 'probe', 'claude.json'), 'utf8'))
    store.c2 = store.c1
    fs.writeFileSync(path.join(dir, '.agentdeck', 'probe', 'claude.json'), JSON.stringify(store))
    const s = probeStatus('claude', 'c2')
    assert.equal(s.status, 'baseline')
    assert.equal('keys' in s ? s.keys : undefined, undefined, 'the status payload does not carry the whole key set')
  })
})

function loadProbeSpec(provider: string) {
  const spec = nullableSpec(provider)
  assert.ok(spec?.sample?.glob, `${provider}: required fixture probe`)
  return { ...spec, sample: { ...spec.sample, glob: spec.sample.glob } }
}
function runProbe(...args: Parameters<typeof nullableRun>) {
  const result = nullableRun(...args)
  assert.ok(result)
  return result
}
function acceptProbe(...args: Parameters<typeof nullableAccept>) {
  const result = nullableAccept(...args)
  assert.ok(result)
  return result
}
function probeStatus(...args: Parameters<typeof nullableStatus>) {
  const result = nullableStatus(...args)
  assert.ok(result)
  return result
}
