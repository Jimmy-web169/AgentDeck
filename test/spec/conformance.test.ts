// Original test group: spec-conformance. Assertions retained during module-path migration.
...
// scripts/check-spec.ts does the work; this keeps it inside `npm test`.
const REPO = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..')

test('spec: descriptors validate and fixture sessions parse to their goldens', () => {
  const r = spawnSync(process.execPath, [path.join(REPO, 'scripts', 'check-spec.ts')], { cwd: REPO, encoding: 'utf8' })
