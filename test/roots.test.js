...
import { HOME, idFor, makeRoots } from '../server/shared/roots.js'

const tmp = () => fs.mkdtempSync(path.join(os.tmpdir(), 'agentdeck-roots-'))
const homeRelative = (dir) => (dir.startsWith(HOME) ? `~${dir.slice(HOME.length)}` : dir)
