...
import { HOME, configDir, isolatedConfig, idFor } from '../server/shared/roots.js'
import { generateFixture, SCENARIOS } from '../scripts/demo/make-fixture.mjs'

// The demo fixture (scripts/demo/make-fixture.mjs) must be servable without
...
const tmp = () => fs.mkdtempSync(path.join(os.tmpdir(), 'agentdeck-demo-'))
...
const providers = async () => (await import('../server/registry.js')).PROVIDERS
const ok = (r, what) => {
...
test('config dir: an explicit dir is authoritative — no seed, no default homes, live per call', async () => {
  ...
  await withConfigDir(dir, () => {
    for (const id of ['claude', 'codex']) assert.deepEqual(P[id].loadRoots(), [], `${id}: empty config dir tracks nothing`)
  ...
  for (const id of ['claude', 'codex']) fs.writeFileSync(path.join(dir, `roots.${id}.json`), JSON.stringify([{ id: idFor(homes[id]), label: `~/.${id}`, dir: homes[id] }]))
  await withConfigDir(dir, async () => {
    for (const id of ['claude', 'codex']) {
      const roots = P[id].loadRoots()
      assert.deepEqual(roots.map((r) => r.dir), [homes[id]], `${id}: only the fixture root`)
      assert.ok(!roots.some((r) => r.dir === path.join(HOME, `.${id}`)), `${id}: the real home is not re-added`)
      const meta = ok(await P[id].dispatch('GET', '/api/roots', q('')), `${id} roots`)
  ...
  await withConfigDir(dir + '-missing', () => {
    for (const id of ['claude', 'codex']) assert.deepEqual(P[id].loadRoots(), [])
  })
})
...
test('generated fixture: v2-highlights parses through both providers (projects, sessions, sub-agents, tokens)', async () => {
  ...
  await withConfigDir(out, async () => {
    for (const id of ['claude', 'codex']) {
      ...
      const { projects } = ok(await P[id].dispatch('GET', '/api/projects', q(`root=${root}`)), `${id} projects`)
      ...
      for (const p of projects) assert.ok(p.cwd && p.cwd.startsWith('C:\\Users\\demo\\code\\'), `${id}: fictional cwd (${p.cwd})`)

      let sessions = 0
      let withSubagents = 0
      for (const p of projects) {
        const body = ok(await P[id].dispatch('GET', '/api/sessions', q(`root=${root}&slug=${encodeURIComponent(p.slug)}`)), `${id} sessions`)
        ...
        for (const s of body.sessions) {
          ...
          if (id === 'codex' && s.childCount > 0) {
            withSubagents++
            const sub = ok(await P.codex.dispatch('GET', '/api/subagents', q(`root=${root}&id=${s.id}`)), 'codex subagents')
            assert.equal(sub.children.length, s.childCount)
            for (const c of sub.children) {
              assert.equal(c.agentRole, 'explorer')
              assert.ok(c.userTurns > 0 && c.tokens.total > 0, 'child rollout summarised')
            }
          }
        }
      }
      ...
      const act = ok(await P[id].dispatch('GET', '/api/activity', q(`root=${root}&days=84`)), `${id} activity`)
      assert.ok(act.totals.sessions > 0 && act.totals.activeDays > 1, `${id}: activity buckets`)
      const hist = ok(await P[id].dispatch('GET', '/api/history', q(`root=${root}`)), `${id} history`)
    ...
    const usage = ok(await P.codex.dispatch('GET', '/api/usage', q(`root=${croot}`)), 'codex usage')
    assert.ok(usage.rateLimits?.primary?.used_percent > 0, 'rate_limits surfaced from the newest token_count')
    const res = ok(await P.codex.dispatch('GET', '/api/resources', q(`root=${croot}`)), 'codex resources')
    assert.match(JSON.stringify(res), /docs-search/, 'config.toml mcp server visible')
    const mem = ok(await P.claude.dispatch('GET', '/api/memory', q(`root=${manifest.rootIds.claude}&slug=${manifest.projects.find((p) => p.name === 'orbit-api').claudeSlug}`)), 'claude memory')
...
test('generated fixture: single-project and empty scenarios; deterministic for a seed', async () => {
  const P = await providers()
  assert.deepEqual(SCENARIOS, ['v2-highlights', 'empty', 'single-project'])
  const base = tmp()
  ...
  await withConfigDir(path.join(base, 'single'), async () => {
    for (const id of ['claude', 'codex']) {
      const { projects } = ok(await P[id].dispatch('GET', '/api/projects', q('')), `${id} projects`)
  ...
  assert.deepEqual(a.sessions.map((s) => [s.id, s.title, s.subagents]), b.sessions.map((s) => [s.id, s.title, s.subagents]), 'same seed + now → same ids')
  const c = generateFixture({ out: path.join(base, 'c'), scenario: 'v2-highlights', seed: 12, now: NOW })
  assert.notDeepEqual(a.sessions.map((s) => s.id), c.sessions.map((s) => s.id), 'another seed → other ids')
...
test('generated fixture: transcripts never mention this machine (home dir, user name)', () => {
  ...
  const needles = [HOME, `\\Users\\${user}\\`, `/home/${user}/`, `/Users/${user}/`].filter((s) => s && s.length > 3)
  const walk = (dir, acc) => {
