// Windows regression coverage: `scripts/statusline-bridge.ts`, the usage-bar bridge: writes `rate-limits.json` from the status-line payload, tolerates PowerShell's BOM, chains the user's original status line unchanged, and never crashes or writes on malformed input.
import { temporaryDirectory } from '../helpers/tmpConfigDir.ts'
// Original test group: windows/statusline-bridge. Assertions retained during module-path migration.
...
const BRIDGE = fileURLToPath(new URL('../../scripts/statusline-bridge.ts', import.meta.url))

function runBridge(input: string, args: string[] = []) {
  return spawnSync(process.execPath, [BRIDGE, ...args], { input, encoding: 'utf8' })
}

function makeCfg() {
  const cfg = temporaryDirectory('adk-bridge-')
  fs.mkdirSync(path.join(cfg, 'projects', 'x'), { recursive: true })
  return cfg
}

function payload(cfg: string) {
