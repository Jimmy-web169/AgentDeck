...
const scroll: { chunks: Buffer[]; bytes: number } = { chunks: [], bytes: 0 }
const SCROLL_MAX = 256 * 1024

function pushScroll(buf: Buffer) {
  scroll.chunks.push(buf)
  scroll.bytes += buf.length
  while (scroll.bytes > SCROLL_MAX && scroll.chunks.length > 1) {
    const removed = scroll.chunks.shift()
    if (!removed) break
    scroll.bytes -= removed.length
  }
}

function spawnPty(cols: number, rows: number, ws: WebSocket) {
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (raw) => {
    ...
    try {
      msg = jsonRecord(JSON.parse(raw.toString()))
    } catch {
      return
    }
    if (msg.t === 's') {
      const cols = Math.max(2, Number(msg.c) | 0) || 80
      const rows = Math.max(2, Number(msg.r) | 0) || 24
    ...
    } else if (msg.t === 'i' && proc && typeof msg.d === 'string') {
      proc.write(msg.d)
    } else if (msg.t === 'r' && proc) {
      resizeTo(Number(msg.c), Number(msg.r))
    }
  })
