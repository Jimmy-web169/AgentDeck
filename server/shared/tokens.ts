...
export function zeroTokens(specific: readonly string[] = []) {
  const t: { input: number; output: number; cacheRead: number; total: number; [key: string]: number } = { input: 0, output: 0, cacheRead: 0, total: 0 }
  for (const k of specific) t[k] = 0
  return t
}

// sum every numeric field `t` carries (common or specific) into `into`
export function addTokens(into: Record<string, number>, t: Record<string, unknown> | null | undefined) {
  if (!t) return into
...
export function withTotal<T extends Record<string, number>>(t: T): T & { total: number }
export function withTotal(t: null): null
export function withTotal(t: undefined): undefined
export function withTotal(t: Record<string, number> | null | undefined): (Record<string, number> & { total: number }) | null | undefined
export function withTotal(t: Record<string, number> | null | undefined) {
  if (!t) return t
  if (typeof t.total === 'number' && t.total > 0) return t
  return { ...t, total: (t.input || 0) + (t.output || 0) + (t.cacheRead || 0) + (t.cacheCreate || 0) }
}

export const tokenFields = (specific: readonly string[] = []) => ({ common: COMMON_TOKEN_FIELDS, specific })
