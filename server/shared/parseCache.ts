...
const MAX_CACHE_BYTES = Number(process.env.AGENTDECK_PARSE_CACHE_BYTES) > 0 ? Number(process.env.AGENTDECK_PARSE_CACHE_BYTES) : 64 * 1024 * 1024
...
const records = new Map<string, CacheEntry>() // path -> { fp, cost, value }
let recordBytes = 0
...
const derived = new Map<string, CacheEntry>() // `${kind}\n${path}` -> { fp, cost, value }
let derivedBytes = 0
 ...
 */
export function fingerprintOf(file: string): Fingerprint {
...
export function etagOf(fp: Fingerprint, extra = '') {
  return `"${fp.key}${extra ? `:${extra}` : ''}"`
}

function dropDerivedFor(file: string) {
  const suffix = `\n${file}`
 ...
 */
export function cachedRecords<T>(file: string, parse: (file: string) => T, fp = fingerprintOf(file)): T {
  const hit = records.get(file)
  if (hit && hit.fp === fp.key) {
    records.delete(file) // LRU bump
    records.set(file, hit)
    // The same parser/kind owns each cache address for its lifetime.
  ...
  while (recordBytes > MAX_CACHE_BYTES && records.size > 1) {
    ...
    const [oldPath, old] = oldest.value
 ...
 */
export function cachedDerived<T>(file: string, kind: string, compute: () => T, fp = fingerprintOf(file)): T {
  const key = `${kind}\n${file}`
  ...
  while (derivedBytes > MAX_CACHE_BYTES && derived.size > 1) {
    const oldest = derived.entries().next()
    if (oldest.done) break
