...
function run(cmd: string, args: string[], { waitExit = false } = {}) {
  ...
  const opts: SpawnOptions = { detached: !waitExit, stdio: 'ignore' }
...
export async function openTool(what: string, cwd: string) {
...
export function pickFolderNative() {
  return new Promise((resolve) => {
    ...
    let out = ''
    let child: import('node:child_process').ChildProcessByStdio<null, import('node:stream').Readable, null>
    try {
