...
export function parseSkillsAdd(input: string, { forceAgent = false } = {}) {
  let s = (input || '').trim()
  ...
  const looksOk = /^[\w.-]+\/[\w.-]+(\/.+)?$/.test(ref) || /^(https?:\/\/|git@)/.test(ref) || ref.startsWith('../../tmp') || ref.startsWith('../..')
...
export function buildSkillsArgv(args: string[], { global, agent }: { global?: boolean; agent: string }) {
...
function cleanOutput(s: string) {
  return (
    s
      // biome-ignore lint/suspicious/noControlCharactersInRegex: These byte patterns decode ANSI terminal control sequences from CLI output.
...
export interface SkillConfig<Pre> {
  agent: string
  envKey: string
  forceAgent?: boolean
  beforeRun?(context: SkillContext): Pre
  afterRun?(context: SkillContext, before: Pre | null): string | null | undefined
}
...
export function runSkillsAdd<Pre>({
  cwd,
  args,
  global,
  configDir,
  config,
}: SkillContext & { cwd: string; args: string[]; config: SkillConfig<Pre> }): Promise<SkillResult> {
  ...
  const env: NodeJS.ProcessEnv = { ...process.env, npm_config_yes: 'true', NO_COLOR: '1' }
  ...
  return new Promise((resolve) => {
    let proc: import('node:child_process').ChildProcessByStdio<null, import('node:stream').Readable, import('node:stream').Readable>
    try {
      const { bin, prefix } = npxSpawnSpec()
      proc = spawn(bin, [...prefix, ...argv], { cwd, env, stdio: ['ignore', 'pipe', 'pipe'] })
    } catch (e) {
      return resolve({ ok: false, command, code: null, output: `failed to start npx: ${e instanceof Error ? e.message : e}` })
    }
    let out = ''
