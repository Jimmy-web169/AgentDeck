...
import { HANDOFF_LAUNCH, type HandoffState, type HandoffHeader, type openHandoffStore } from './handoffStore.ts'
...
const publicResult = (v: HandoffState) => ({
  id: v.id,
  filename: v.filename,
  file: v.file,
  bytes: v.bytes,
  history: v.history,
  state: v.state || 'exported',
  terminal: v.terminal || null,
  error: v.error || null,
  target: v.target || null,
})
interface Provider extends HttpProvider {
  capabilities?: { interactiveContext?: boolean }
...
export function createHandoffService({
  ...
  getStore: typeof openHandoffStore
  terminals?: () => TerminalMetadata[]
}) {
  const destinations = () =>
    Object.values(providers)
      .filter((p) => p.capabilities?.interactiveContext)
      .flatMap((p) => p.loadRoots().map((r) => ({ provider: p.id, root: r.id, rootLabel: r.label || r.id })))
  const exportHistory = async (input: unknown) => {
    const body = jsonRecord(input),
      source = jsonRecord(body.source),
      task = body.task ?? null
    if (!source || typeof source.provider !== 'string' || typeof source.root !== 'string' || typeof source.id !== 'string' || !source.id)
      throw err(400, 'Choose a saved source conversation.')
    ...
    const locator = {
      ...
      root: source.root,
      id: source.id,
      ...(typeof source.slug === 'string' && source.slug ? { slug: source.slug } : {}),
    }
    ...
    const header: HandoffHeader = {
      ...
      handoff: {
        ...
        task: task?.trim() || null,
        history: {
          includes: ['user_messages', 'assistant_messages', 'subagent_conversations'],
          excludes: ['tool_calls', 'tool_outputs', 'thinking', 'system_instructions'],
          complete: captured.complete,
          warnings: captured.warnings,
          conversations: captured.conversationCount,
  ...
  const dispatch = async (input: unknown) => {
    const { id } = jsonRecord(input),
      target = jsonRecord(jsonRecord(input).target)
    const store = getStore(),
      current = store.status(id)
    ...
    const p = providers[target.provider],
      root = p?.loadRoots().find((r) => r.id === target.root)
    ...
    if (!cwd || !path.isAbsolute(cwd) || !fs.statSync(cwd, { throwIfNoEntry: false })?.isDirectory())
      throw err(409, 'The original working folder is unavailable. The JSONL is still saved; choose a valid local folder when using it on another machine.')
    const { claimed, value } = store.claim(id, { provider: target.provider, root: target.root, cwd })
    if (!claimed) return publicResult(value)
    try {
      const response = await p.dispatch('POST', '/api/terminal', new URLSearchParams(), {
        root: target.root,
        cwd,
        launchId: value.launchId,
        handoffExportId: id,
        [HANDOFF_LAUNCH]: value,
        title: `Continue · ${value.projectName}`,
      })
      const body = jsonRecord(response.body)
      if (response.status !== 200 || typeof body.key !== 'string' || !body.key || body.launchId !== value.launchId)
        throw err(502, String(body.error || 'Terminal did not return the exact launch identity.'))
      return publicResult(store.finish(id, { state: 'launched', terminal: { ...body, key: body.key, provider: target.provider, root: target.root, cwd } }))
    } catch (e) {
      return publicResult(store.finish(id, { state: 'unknown', error: e instanceof Error ? e.message : String(e) }))
    }
  }
  const status = (input: unknown) => {
    ...
    if (['dispatching', 'unknown'].includes(current.state || '')) {
      const terminal = terminals().find(
        (t): t is TerminalMetadata & { key: string } =>
          typeof t.key === 'string' && !!t.key && t.launchId === current.launchId && t.provider === current.target?.provider && t.root === current.target?.root
      )
