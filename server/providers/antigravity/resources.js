...
const listFiles = (dir, ext) => {
  try {
    return fs.readdirSync(dir, { withFileTypes: true }).filter((e) => e.isFile() && e.name.endsWith(ext)).map((e) => e.name)
  } catch {
    return []
  }
}
...
function hooksFrom(file, scope) {
  ...
  const events = Object.entries(map)
    ...
    .map(([k, v]) => ({ event: k, handlers: Array.isArray(v) ? v.length : Object.keys(v).length }))
  return { scope, sourcePath: file, events }
}

function rulesIn(base, scope) {
  const out = []
...
function pluginsIn(dir, scope, enabledMap) {
  const out = []
  for (const name of listDirs(dir)) {
    ...
    out.push({
      ...
      path: pdir,
      scope,
      enabled: enabledMap?.[pdir]?.enabled ?? enabledMap?.[name]?.enabled ?? true,
...
function workspaceConfigDir(base) {
...
export function inventory(scope, base, rootDir) {
  ...
  const cfg = configDir()
  const enabledMap = readJson(path.join(cfg, 'config.json'))?.plugins || {}
  ...
  return {
    ...
    permissions: readJson(path.join(cfg, 'config.json'))?.userSettings?.globalPermissionGrants || null,
    readOnly: true,
  }
}
