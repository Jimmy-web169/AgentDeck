...
export interface ResourceBody extends Record<string, unknown> {
...
function splitToml(toml: string | null) {
  const top: string[] = []
...
function kvPairs(lines: string[], keys?: string[]) {
...
function configSummary(toml: string | null) {
...
function mcpServers(toml: string | null) {
...
function hookEvents(toml: string | null, hooksJson: string | null) {
  ...
  if (hooksJson) {
    try {
      const o = fields(JSON.parse(hooksJson) as unknown)
      for (const k of Object.keys(fields(o.hooks))) events.add(k)
    } catch {}
  }
  return [...events].sort()
}
...
function customAgents(agentsDir: string) {
...
function listRules(rulesDir: string) {
...
function listSkills(dirs: SkillDirectory[]) {
  const out = []
  const seen = new Set()
...
function readAgentsMd(dir: string) {
...
function httpish(status: number, message: string) {
...
const q = (s: unknown) => JSON.stringify(String(s ?? '')) // TOML/JSON-safe double-quoted string

function scopePaths(scope: string, base: string) {
...
function buildAgentToml(f: ResourceBody) {
...
function buildMcpBlock(f: ResourceBody) {
...
function addHook(file: string, f: ResourceBody & { event: string }) {
  let doc: Record<string, unknown> = {}
  try {
    doc = fields(JSON.parse(fs.readFileSync(file, 'utf8')) as unknown)
  } catch {}
  if (!doc || typeof doc !== 'object') doc = {}
  if (!doc.hooks || typeof doc.hooks !== 'object') doc.hooks = {}
  const hooks = fields(doc.hooks)
  if (!Array.isArray(hooks[f.event])) hooks[f.event] = []
  const handler: Record<string, unknown> = { type: 'command', command: f.command }
  ...
  ;(hooks[f.event] as unknown[]).push(group)
  fs.writeFileSync(file, JSON.stringify(doc, null, 2) + '\n')
}

// Create a resource at the given scope. Returns the path written.
export function createResource(scope: string, base: string, body: ResourceBody) {
  ...
  switch (body.kind) {
    ...
    case 'hook': {
      if (typeof body.event !== 'string' || !HOOK_EVENTS.includes(body.event)) throw httpish(400, 'unknown hook event')
      if (!String(body.command || '').trim()) throw httpish(400, 'hook command is required')
      fs.mkdirSync(p.codexDir, { recursive: true })
      addHook(p.hooksJson, { ...body, event: body.event })
      return p.hooksJson
    }
    case 'mcp': {
...
export function inventory(scope: string, baseDir: string) {
...
function skillDirsFor(scope: string, base: string): SkillDirectory[] {
  const codexDir = scope === 'project' ? path.join(base, '.codex') : base
...
function findSkillDir(dirs: SkillDirectory[], name: string) {
  for (const { dir, system } of dirs) {
...
function findAgentFile(agentsDir: string, name: string) {
...
function findRuleFile(rulesDir: string, name: string) {
...
function stripTomlSection(toml: string, target: string) {
  const out = []
  let skipping = false
  let inBlock = false
  let removed = false
...
export async function deleteResource(scope: string, base: string, kind: string, rawName: unknown) {
  ...
  switch (kind) {
    ...
    case 'hook': {
      ...
      try {
        const doc = fields(JSON.parse(fs.readFileSync(p.hooksJson, 'utf8')) as unknown)
        if (doc?.hooks && name in fields(doc.hooks)) {
          delete fields(doc.hooks)[name]
