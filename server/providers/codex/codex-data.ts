...
function sqliteJson(dbFile: string, sql: string): Record<string, unknown>[] | null {
  ...
  try {
    ...
    return out.trim() ? jsonArray(JSON.parse(out) as unknown).map(jsonRecord) : []
  } catch {
    return null
  }
}
...
export function readMemories(rootDir: string) {
  ...
  for (const t of sqliteJson(path.join(rootDir, 'state_5.sqlite'), 'SELECT id, title, cwd FROM threads;') || []) threads[String(t.id)] = t
  const memories = rows.map((r) => ({
    threadId: r.thread_id,
    title: threads[String(r.thread_id)]?.title || null,
    cwd: threads[String(r.thread_id)]?.cwd || r.rollout_slug || null,
...
function pluginEnabledMap(configFile: string) {
...
function listSkillNames(dir: string) {
  try {
    return fs
  ...
  for (const mk of ls(cacheDir)) {
    ...
    for (const src of ls(path.join(cacheDir, mk.name))) {
      ...
      for (const hash of ls(path.join(cacheDir, mk.name, src.name))) {
        ...
        try {
          m = jsonRecord(JSON.parse(txt) as unknown)
        } catch {
          continue
        }
        const repo = typeof m.repository === 'string' ? m.repository : jsonRecord(m.repository).url || null
        installed.push({
          name: optionalString(m.name) || hash.name,
          version: m.version || null,
          description: m.description || jsonRecord(m.interface).shortDescription || '',
          marketplace: mk.name,
          source: src.name,
          ...
          displayName: jsonRecord(m.interface).displayName || m.name || null,
        })
      }
    }
  }
