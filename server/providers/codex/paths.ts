...
import { jsonRecord, optionalString, optionalTimestamp, numberOrZero } from '../../shared/json.ts'
...
const CONFIG_PATH = () => stateFile('roots', 'codex')

function hasSessions(dir: string) {
  try {
...
function defaultRoots(): Root[] {
  const out: Root[] = []
  for (const p of [process.env.CODEX_HOME, path.join(HOME, '.codex')].filter((value): value is string => typeof value === 'string' && !!value)) {
...
const _roots = makeRoots({
  ...
  autodetectSeed,
  defaultRoots,
  dataProbe: (dir) => ({ hasSessions: hasSessions(dir) }),
...
export function sessionsDir(rootDir: string) {
...
export function idFromFilename(name: string) {
...
function walkSessionFiles(dir: string, out: FileEntry[]): FileEntry[] {
...
interface IndexEntry extends FileEntry, SessionHead {}
interface SessionIndex {
  at: number
  byId: Map<string, IndexEntry>
  byCwd: Map<string, IndexEntry[]>
}
const EMPTY_HEAD: SessionHead = {
  cwd: null,
  startTs: null,
  branch: null,
  isSubagent: false,
...
function readHead(file: string): SessionHead {
  try {
    ...
    for (const line of lines) {
      ...
      try {
        rec = jsonRecord(JSON.parse(s))
      } catch {
        continue
      }
      const p = rec.payload && typeof rec.payload === 'object' ? jsonRecord(rec.payload) : rec
      if (!head.startTs && (rec.timestamp || p.timestamp)) head.startTs = optionalTimestamp(rec.timestamp) || optionalTimestamp(p.timestamp) || null
      if (head.cwd == null && typeof p.cwd === 'string') head.cwd = p.cwd
      if (head.branch == null) head.branch = optionalString(jsonRecord(p.git).branch) ?? null
      // subagent linkage lives on the session_meta record (same line as cwd)
      const rawSpawn = jsonRecord(jsonRecord(p.source).subagent).thread_spawn
      const spawn = jsonRecord(rawSpawn)
      if (rawSpawn) {
        head.isSubagent = true
        head.parentId = optionalString(spawn.parent_thread_id) || head.parentId
        head.agentRole = optionalString(spawn.agent_role) || optionalString(p.agent_role) || head.agentRole
        head.agentNickname = optionalString(spawn.agent_nickname) || optionalString(p.agent_nickname) || head.agentNickname
        ...
        // (see src/components/codex/subagentAdapter.ts)
        head.agentPath = typeof spawn.agent_path === 'string' ? spawn.agent_path : head.agentPath
        head.depth = numberOrZero(spawn.depth) || head.depth
      } else if (p.thread_source === 'subagent') {
        head.isSubagent = true
        head.agentRole = head.agentRole || optionalString(p.agent_role) || null
        head.agentNickname = head.agentNickname || optionalString(p.agent_nickname) || null
      }
...
export function buildIndex(rootDir: string): SessionIndex {
  ...
  for (const f of files) {
    ...
    const entry = {
      ...
      mtimeMs: f.mtimeMs,
      cwd: head.cwd,
      startTs: head.startTs,
      branch: head.branch,
      isSubagent: head.isSubagent,
...
function tailRateLimits(file: string) {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (p.rate_limits)
        return {
          ...
          ts: optionalTimestamp(rec.timestamp) || optionalTimestamp(p.timestamp) || null,
          sessionId: idFromFilename(path.basename(file)),
        }
    }
  } catch {
    // fall through
...
export function latestRateLimits(rootDir: string) {
  const { byId } = buildIndex(rootDir)
...
export function latestCliVersion(rootDir: string) {
  const { byId } = buildIndex(rootDir)
