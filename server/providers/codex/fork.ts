...
const cleanUser = (raw: unknown) =>
  String(raw || '')
    .replace(ENV_WRAP, '')
    .replace(INSTR_WRAP, '')
    .trim()

const bodyText = (p: Record<string, unknown>) => {
  const c = p.content
  if (Array.isArray(c)) return c.map((b) => (typeof b === 'string' ? b : jsonRecord(b).text || '')).join('\n')
  return String(c || '')
}

/** @returns {string[]} kept lines · @throws {Error} e.code='CUT_NOT_FOUND' | 'EMPTY_FORK' */
export function forkLines(rawLines: string[], cut: number | null | undefined, newId: string): string[] {
  const lines = rawLines.filter((l) => l.trim())
  const recs: unknown[] = lines.map((l) => {
    try {
      return JSON.parse(l)
    } catch {
      return null
  ...
  const hasEventUser = recs.some((value) => {
    const r = jsonRecord(value)
    return r.type === 'event_msg' && jsonRecord(r.payload).type === 'user_message'
  })
  const isUserPrompt = (value: unknown) => {
    const r = jsonRecord(value)
    if (!r) return false
    if (hasEventUser) return r.type === 'event_msg' && jsonRecord(r.payload).type === 'user_message' && !!cleanUser(jsonRecord(r.payload).message)
    const p = r.payload && typeof r.payload === 'object' ? jsonRecord(r.payload) : r
  ...
  const isScaffold = (value: unknown) => {
    const r = jsonRecord(value)
    if (!r) return false
    if (r.type === 'event_msg' && jsonRecord(r.payload).type === 'task_started') return true
  ...
  if (cut) {
    ...
    if (cutIdx < 0) {
      const e = Object.assign(new Error('cut point not found in transcript'), { code: 'CUT_NOT_FOUND' })
      throw e
    }
  ...
  if (!end || !recs.slice(0, end).some(isUserPrompt)) {
    const e = Object.assign(new Error('fork would be empty'), { code: 'EMPTY_FORK' })
    throw e
  }
  const out = []
  for (let i = 0; i < end; i++) {
    const r = jsonRecord(recs[i])
