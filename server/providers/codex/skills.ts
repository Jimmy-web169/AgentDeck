import type { SkillConfig } from '../../shared/skills.ts'
import fs from 'node:fs'
import os from 'node:os'
import path from 'node:path'

// Codex skill-install config for the shared skills runner (see ../../shared/skills.ts).
...
function listSkillDirs(dir: string): Set<string> {
  try {
    return new Set(
      fs
...
function relocateCodexSkills(before: ReadonlySet<string>, destSkillsDir: string) {
...
export const SKILL_CONFIG: SkillConfig<Set<string> | null> = {
  agent: 'codex',
  envKey: 'CODEX_HOME',
  forceAgent: true,
