...
export type WorkspaceItem = Target & { folderId?: string; name?: string | null }
...
import { baseName, shortPath } from './paths.ts'
...
import { normalizeColor } from './accent.ts'
export const WORKSPACE_ICON_KEYS = ['layers', 'folder', 'star', 'bolt', 'rocket', 'flask', 'book', 'briefcase', 'globe', 'code', 'heart', 'tag']

import { projectKey, projectSessionKey, folderItemKey, sourceKey as identitySourceKey } from '../../shared/identity.ts'
export { projectKey }
export const isFolderItem = (it: WorkspaceItem | null | undefined) => it?.kind === 'folder'
export const folderItem = (folder: Pick<Folder, 'id' | 'cwd' | 'name'>) => ({
  kind: 'folder',
  folderId: folder.id,
  cwd: folder.cwd,
  name: folder.name || baseName(folder.cwd),
})
export const itemKey = (it: WorkspaceItem) => (isFolderItem(it) ? folderItemKey(it.folderId || '') : projectSessionKey(it))
export const sourceKey = (it: WorkspaceItem) => identitySourceKey(it?.provider || '', it?.root || '')

function normItem(it: WorkspaceItem | null | undefined): WorkspaceItem | null {
  if (it && isFolderItem(it))
    return typeof it.folderId === 'string' && it.folderId && typeof it.cwd === 'string' && it.cwd
      ? { kind: 'folder', folderId: it.folderId, cwd: it.cwd, name: it.name || baseName(it.cwd) }
      : null
  if (!it?.provider || !it.root || !it.slug) return null
...
function load(): Workspace[] {
  try {
    const arr: (Partial<Workspace> & { projects?: WorkspaceItem[] })[] = JSON.parse(localStorage.getItem(KEY) || '[]')
    if (!Array.isArray(arr)) return []
    return arr
      .filter((w): w is Workspace & { projects?: WorkspaceItem[] } => !!w && typeof w.id === 'string' && typeof w.name === 'string')
      .map((w) => ({
        id: w.id,
        name: w.name,
        at: w.at || 0,
        ...
        items: [...(Array.isArray(w.items) ? w.items : []), ...(Array.isArray(w.projects) ? w.projects.map((p) => ({ ...p, kind: 'project' })) : [])]
          .map(normItem)
...
export function createWorkspace(name: string, items: WorkspaceItem[] = []) {
  const seen = new Set()
  const list = []
...
export function setWorkspaceColor(id: string, color: unknown) {
  const c = normalizeColor(color)
  save(workspaces.map((w) => (w.id === id ? { ...w, color: c } : w)))
}

export function setWorkspaceIcon(id: string, icon: string) {
...
export function workspaceHolding(raw: WorkspaceItem, list = workspaces, folders: Folder[] = []) {
  const it = normItem(raw)
  if (!it) return null
  const k = itemKey(it)
  return (
    list.find(
      (w) => w.items.some((x) => itemKey(x) === k) || (!isFolderItem(it) && folderProjects(w, folders).some((p) => projectKey(p) === projectKey(it)))
    ) || null
  )
}

export function addToWorkspace(id: string, raw: WorkspaceItem) {
  const it = normItem(raw)
  if (!it) return
  const k = itemKey(it)
...
export function removeFromWorkspace(id: string, raw: WorkspaceItem) {
  const it = normItem(raw)
  if (!it) return
  const k = itemKey(it)
...
export const inWorkspace = (w: Workspace, raw: WorkspaceItem) => {
  const it = normItem(raw)
...
export function folderProjects(w: Workspace, folders: Folder[] = []) {
  const ids = new Set(w.items.filter(isFolderItem).map((it) => it.folderId))
  return folders
    .filter((f) => f.resolved && ids.has(f.id))
    .flatMap((f) => f.sources.filter((s) => s.slug).map((s) => normItem({ ...s, kind: 'project', cwd: s.cwd || f.cwd, project: baseName(f.cwd) })))
    .filter((item): item is WorkspaceItem => item !== null)
}
...
export function standaloneWorkspaceItems(w: Workspace, folders: Folder[] = []) {
  const covered = new Set(folderProjects(w, folders).map(projectKey))
  return w.items.filter((it: Target | null | undefined) => !isFolderItem(it) && !covered.has(projectKey(it)))
}

// Distinct provider/root sources, including current folder membership.
export const workspaceSources = (w: Workspace, folders: Folder[] = []) => {
  const seen = new Map()
...
export const normCwd = (c: string | null | undefined) =>
  String(c || '')
    .replace(/[\\/]+$/, '')
    .replace(/\\/g, '/')
    .toLowerCase()
...
function loadAdoptions(): Adoption[] {
  try {
    ...
    return Array.isArray(arr) ? arr.filter((a) => a?.ws && a.provider && a.root && a.cwd && Date.now() - (a.at || 0) < ADOPT_TTL) : []
  } catch {
    return []
  }
}
function saveAdoptions(list: Adoption[]) {
  try {
...
export function rememberAdoption(source: WorkspaceItem, target: Target, cwd: string) {
...
export function adoptPendingProjects(indexProjects: WorkspaceItem[] = []) {
  ...
  for (const a of list) {
    ...
    if (workspaces.some((w) => w.id === a.ws))
      addToWorkspace(a.ws, {
        kind: 'project',
        provider: p.provider,
        root: p.root,
...
export function suggestWorkspaces(indexProjects: WorkspaceItem[] = [], current = workspaces, folders: Folder[] = []) {
  ...
  const byCwd = new Map<string, WorkspaceItem[]>()
  for (const p of indexProjects) {
    if (!p.cwd) continue
    const k = normCwd(p.cwd)
    const projects = byCwd.get(k) || []
    byCwd.set(k, projects)
    projects.push(p)
  }
  const out = []
  for (const [, list] of byCwd) {
    ...
    if (list.some((p: Target | null | undefined) => grouped.has(projectKey(p)))) continue
    out.push({
      name: baseName(list[0].cwd),
      cwd: list[0].cwd,
      items: list.map((p) => normItem({ ...p, kind: 'project' })).filter((item): item is WorkspaceItem => item !== null),
      sources: list.map((p) => ({ provider: p.provider, root: p.root, rootLabel: p.rootLabel })),
    })
  }
