...
// Main area for an id-addressed provider (Codex, Antigravity): a session is
// opened by id and its project (cwd) derived. The provider hands over its API
...
export default function makeIdApp({ providerId, api, Conversation, ResourcesView, SubagentsView, MemoryView, TerminalPanel, RateLimitsBar, Stats, rawTypeOf, sessionTabs, subagentAdapter = null, docsUrl, docsTitle, usageInfo }) {
  ...
  function App({ active: appActive = true, providers, scopes, onOpenHome, onOpenSession, pendingOpen, onConsumedPending, onNavigate }) {
    ...
    const report = useCallback((t, opts) => onNavigateRef.current?.(providerId, targetOf(t), opts), [targetOf])
    ...
    const subagentCtx = useMemo(() => (root && activeId ? { root, id: activeId, provider: providerId, api, adapter: subagentAdapter } : null), [root, activeId])
    ...
    useEffect(() => {
      if (!appActive) return
      const list = (scopes || []).filter((s) => s.provider === providerId)
    ...
    useEffect(() => {
      ...
      if (!pendingOpen.id && (pendingOpen.draft || pendingOpen.kind === 'tmux' || pendingOpen.newConversation) && (pendingOpen.slug || pendingOpen.cwd)) {
        ...
        setTermDraft(
          pendingOpen.slug
            ? { root: pendingOpen.root, slug: pendingOpen.slug, cwd: pendingOpen.cwd || pendingOpen.slug, title: pendingOpen.title || 'New conversation' }
    ...
    // ---- account usage limits (5-hour / weekly) ----
    ...
    useEffect(() => {
      ...
      es.onmessage = (e) => {
        ...
        const providerChanges = (msg.changes || []).filter((c) => c.provider === providerId)
  ...
  return (
    <div className="h-full flex items-center justify-center text-center text-zinc-600">
      <div>
        ...
        <div className="text-[12px] mt-1 text-zinc-700">Live updates stream in as {label} writes to disk.</div>
