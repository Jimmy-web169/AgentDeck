...
// result (paired by adjacency in server/providers/antigravity/parser.js) is a
...
export function resolve(part, ctx) {
  ...
  if (id) c = free.find((x) => x.id === id) || null
  if (!c && ctx.ev?.ts) {
...
export function fetchTranscript(item, ctx) {
  return (ctx.api || antigravityApi).session(ctx.root, item.childId)
}

export function cacheKey(item, ctx) {
  return `antigravity|${ctx.root}|${item.childId}`
}
