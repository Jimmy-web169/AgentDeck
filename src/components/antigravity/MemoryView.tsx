  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      ...
      <p className="text-[12px] text-zinc-500 mb-4">
        ...
        <a href="https://antigravity.google/docs/cli/artifacts" target="_blank" rel="noreferrer" className="text-sky-400 hover:text-sky-300">
          Reviewing artifacts ↗
        </a>
      </p>
      {mems.length === 0 ? (
        ...
        mems.map((m) => (
          <div key={m.id} className="mb-3 rounded-lg border border-zinc-800 bg-ink-900/40">
            <button
              type="button"
              onClick={() => setOpen((o) => (o === m.id ? null : m.id || null))}
              className="w-full text-left px-3 py-2.5 flex items-center gap-2 hover:bg-ink-700/40"
            >
