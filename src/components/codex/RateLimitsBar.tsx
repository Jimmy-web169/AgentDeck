import type { RateWindow, RateLimitsProps } from '../../api/models.ts'
import { fmtRelative } from '../../lib/format.ts'
import { useProviderLabel } from '../../api/index.ts'
...
const textColor = (p: number) => (p >= 80 ? 'text-red-300' : p >= 50 ? 'text-amber-300' : 'text-emerald-300')
const barColor = (p: number) => (p >= 80 ? 'bg-red-400' : p >= 50 ? 'bg-amber-400' : 'bg-emerald-400')
const winLabel = (m: number) => (!m ? '·' : m >= 10080 ? '7d' : m >= 1440 ? `${Math.round(m / 1440)}d` : `${Math.round(m / 60)}h`)

// when (epoch ms) does this window reset? supports both snapshot shapes
function resetAtMs(w: RateWindow, ts?: string | number | null) {
...
function Meter({ w, ts }: { w?: RateWindow; ts?: RateLimitsProps['ts'] }) {
  if (!w || typeof w.used_percent !== 'number') return null
  const p = Math.round(w.used_percent)
  const label = winLabel(w.window_minutes || 0)
...
export default function RateLimitsBar({ usage, ts }: RateLimitsProps) {
  const label = useProviderLabel()
  if (!usage || (!usage.primary && !usage.secondary)) return null
  const age = ts ? fmtRelative(ts) : null
  return (
    <div className="flex items-center gap-3 text-[11px]" title={`${label} usage — newest snapshot from your sessions${age ? `, as of ${age}` : ''} (not live)`}>
