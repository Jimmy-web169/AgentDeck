import type { AgentSummary, ConversationData, SubagentsProps } from '../../api/models.ts'
...
import { useProviderId, useSubagents, useSession } from '../../api/index.ts'
...
function AgentRow({ c, onOpen }: { c: AgentSummary; onOpen: (agent: AgentSummary) => void }) {
  const title = c.title || c.firstPrompt || `agent ${c.id.slice(0, 8)}`
  return (
    <button
      type="button"
      onClick={() => onOpen(c)}
      className="w-full text-left px-3 py-2.5 hover:bg-ink-700/50 flex items-center gap-2.5 border-b border-zinc-800/50 group"
    >
      ...
      {(c.depth || 0) > 1 && <span className="text-[10px] text-zinc-600 shrink-0">depth {c.depth}</span>}
      <ContextMeter summary={c} label="ctx" />
      {!!c.tokens?.output && c.tokens.output > 0 && <span className="text-[10px] text-zinc-600 font-mono shrink-0">↓{fmtTokens(c.tokens.output)}</span>}
...
function TranscriptModal({ tx, onClose, onOpenSession }: { tx: Transcript; onClose: () => void; onOpenSession?: (id: string) => void }) {
...
export default function SubagentsView({ root, parent, active = true, onOpenSession }: SubagentsProps) {
  const providerId = useProviderId()
  const { data, error: failure } = useSubagents({ provider: providerId, root: root || '', id: parent?.id || '' }, { enabled: active })
  const error = failure?.message
  const [selected, setSelected] = useState<{ root?: string | null; parentId?: string; c: AgentSummary } | null>(null)
  ...
  useEffect(() => setSelected(null), [root, parent?.id])
  const current = selected?.root === root && selected?.parentId === parent?.id ? selected?.c : null
  const transcript = useSession({ provider: providerId, root: root || '', id: current?.id || '' }, { enabled: active && !!current })
  const tx = current ? { c: current, data: transcript.data, loading: transcript.isPending, error: transcript.error?.message } : null
  const open = (c: AgentSummary) => setSelected({ root, parentId: parent?.id, c })
    ...
    <div className="mx-auto max-w-3xl px-4 py-6">
      <div className="mb-3">
        ...
        <p className="text-[12px] text-zinc-500 mt-0.5">
          {children.length} subagent thread{children.length === 1 ? '' : 's'} spawned by this session. Each is its own rollout — click to view its full
          transcript (context, prompts, tool calls).
        </p>
      </div>
      {children.length === 0 ? (
        ...
        <div className="rounded-lg border border-zinc-800 bg-ink-900/40 overflow-hidden">
          {children.map((c) => (
            <AgentRow key={c.id} c={c} onOpen={open} />
          ))}
        </div>
      )}
      {tx && <TranscriptModal tx={tx} onClose={() => setSelected(null)} onOpenSession={onOpenSession} />}
    </div>
  )
}
