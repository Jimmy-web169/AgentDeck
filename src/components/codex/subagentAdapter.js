...
// server exposes it as child.agentPath (server/providers/codex/paths.js
...
const lastSeg = (p) => (typeof p === 'string' ? p.split('/').filter(Boolean).pop() || null : null)
...
export function resolve(part, ctx) {
  ...
  const role = c.agentRole || lastSeg(c.agentPath) || input.task_name || 'agent'
