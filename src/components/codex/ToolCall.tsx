...
const record = (value: unknown): Input => (value && typeof value === 'object' ? (value as Input) : {})
import { useState } from 'react'
import { Pre, Field, MdBox, asText, isHuge } from '../shared/ToolCallParts.tsx'
...
function commandText(input: Input) {
  if (!input) return ''
...
const TOOL_META: Record<string, { color: string; badge: string; preview: (input: Input) => unknown }> = {
  shell: SHELL,
  shell_command: SHELL,
  local_shell: SHELL,
  ...
  apply_patch: {
    ...
    preview: (i) => (Array.isArray(i.changes) ? i.changes.map((c) => `${c.kind} ${c.path}`).join(', ') : String(i.patch || '').split('\n')[0]),
  },
  update_plan: {
    color: 'text-sky-300',
    badge: 'bg-sky-500/15',
    preview: (i) => `${(Array.isArray(i.plan) ? i.plan : Array.isArray(i.items) ? i.items : []).length} steps`,
  },
  ...
  wait_agent: { ...AGENT, preview: (i) => `wait for ${(Array.isArray(i.targets) ? i.targets : []).length} agent(s)` },
}

function previewOf(name: string, rawInput: unknown) {
...
function PatchBlock({ text }: { text: unknown }) {
  const t = asText(text)
        ...
        const cls = l.startsWith('+')
          ...
          : l.startsWith('-')
            ...
            : /^(@@|\*\*\*)/.test(l)
              ...
              : 'text-zinc-300'
        return (
          // biome-ignore lint/suspicious/noArrayIndexKey: Patch lines and plan steps have ordinal identity inside one tool call and may repeat verbatim.
...
function prettyInput(name: string, rawInput: unknown) {
  const input = record(rawInput)
...
export default function ToolCall({ part }: { part: import('../../../shared/types.d.ts').ToolCallPart }) {
  ...
  return (
    <div className={`rounded-lg border ${err ? 'border-red-500/40' : 'border-zinc-700/70'} bg-ink-700/60 my-2`}>
      <button type="button" onClick={() => setOpen(!open)} className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-ink-600/40 rounded-lg">
      ...
      {open && (
        <div className="px-3 pb-3">
          ...
          {result && (
            <>
              <div className={`text-[11px] uppercase tracking-wide mt-2 ${err ? 'text-red-400' : 'text-zinc-500'}`}>output{err ? ' (error)' : ''}</div>
