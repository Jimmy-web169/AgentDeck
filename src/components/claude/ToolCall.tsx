...
import { Pre, Field, MdBox, asText } from '../shared/ToolCallParts.tsx'
...
function shellPreview(i: Input) {
  return typeof i.command === 'string' ? i.command : ''
}

function grepPreview(i: Input) {
...
function DiffBlock({ oldText, newText }: { oldText: unknown; newText: unknown }) {
  return (
...
const INPUT_VIEWS: Record<string, (input: Input) => React.ReactNode> = {
  Bash: shellView,
    ...
    if (i.offset != null || i.limit != null) {
      ...
      lines =
        Number.isFinite(off) && Number.isFinite(lim)
          ? `${off} – ${off + lim - 1}`
          : [i.offset != null ? `offset ${asText(i.offset)}` : null, i.limit != null ? `limit ${asText(i.limit)}` : null].filter(Boolean).join(', ')
    }
    return (
      <>
...
function renderInput(name: string, input: unknown) {
