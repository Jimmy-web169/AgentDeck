...
import { WORKSPACE_ICON_KEYS } from '../../lib/workspaces.js'
...
export function WorkspaceIcon({ icon, className = 'w-3.5 h-3.5' }) {
  return (GLYPHS[icon] || GLYPHS.layers)(className)
}

// One quiet row (current glyph, "Icon", "change") that unfolds a grid of the
// choices — the same shape as the colour row next to it.
export function IconField({ value, onPick, accentStyle = null }) {
  const [open, setOpen] = useState(false)
  const current = WORKSPACE_ICON_KEYS.includes(value) ? value : 'layers'
  return (
    <div className="py-0.5">
      <button onClick={() => setOpen((o) => !o)} data-icon-chip className="w-full flex items-center gap-2 py-1 text-left group/chip" title={open ? 'Close' : 'Change icon'}>
