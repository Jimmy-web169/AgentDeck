...
interface BriefPreview extends BriefContext {
  need: string
  providerLabel: string
  cwd?: string | null
  docsIndex?: string | null
  kinds?: { name: string; docs: string }[]
}
...
const KIND_LABELS: Record<string, string> = {
  ...
  skills: 'skills',
  skill: 'skills',
  commands: 'custom commands / prompts',
  workflows: 'workflows',
  rules: 'rules / project memory',
  'output-styles': 'output styles',
  claudeMd: 'CLAUDE.md instructions',
  agentsMd: 'AGENTS.md instructions',
  mcpJson: 'MCP servers',
  mcp: 'MCP servers',
  hook: 'hooks',
  hooks: 'hooks',
  ...
  settingsLocalJson: 'local settings',
  config: 'config file',
  permissions: 'permissions',
}
export function providerDocs(providerId: string) {
...
export function composeBriefPreview({ need, providerLabel, kind, filePath, content, docs, cwd, docsIndex = null, kinds = [] }: BriefPreview) {
  ...
  lines.push(
    ...
    '2. Ask, don’t edit: explain each building block that applies and ask whether I want it, one question at a time.',
    '3. Search the skills ecosystem (find-skills) for what my intent needs and propose matches.',
    '4. Show the plan, wait for a yes.',
    '5. Change only what the plan names; show the diff; explain every field.'
  )
  ...
  if (content)
    lines.push(
      '',
      `## Current content of ${filePath || 'the file'}`,
      '',
      '```',
...
export default function HandoffDialog({
  ...
  const [done, setDone] = useState<Awaited<ReturnType<ProviderClient['terminal']>> | null>(null) // { key, reused, brief }
  ...
  const brief = useMemo(
    () =>
      composeBriefPreview({
        ...
        content: includeContent ? context.content : null,
        docs: context.docs,
        cwd,
        docsIndex,
        kinds,
  ...
  const go = async () => {
    ...
    setErr(null)
    try {
      const body: Parameters<ProviderClient['terminal']>[0] = {
        root,
        title: `✦ ${need.trim().slice(0, 48)}`,
        brief: {
          need: need.trim(),
          kind: context.kind || null,
          filePath: context.filePath || null,
          content: includeContent ? context.content || null : null,
          docs: context.docs || null,
          docsIndex,
          kinds,
        },
      }
      ...
      shellActions.openTerminal(liveTarget({ ...res, provider: providerId, root, title: typeof body.title === 'string' ? body.title : undefined }))
    } catch (e) {
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions lint/a11y/useKeyWithClickEvents: The backdrop supplements the existing Close button and Escape handler.
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-6" onClick={onClose}>
      {/* biome-ignore lint/a11y/useKeyWithClickEvents: The dialog only stops backdrop clicks; its controls and Escape stack handle keyboard interaction. */}
      <div
        role="dialog"
        aria-modal="true"
        aria-label={`Ask ${providerLabel} to do it`}
        className="w-[640px] max-w-[95vw] max-h-[88vh] bg-ink-800 border border-zinc-700 rounded-xl shadow-2xl flex flex-col"
      ...
      >
        <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-800 shrink-0">
          <div className="min-w-0">
            ...
            <div className="text-[11px] text-zinc-500 truncate">
              {context.kind
                ? `${context.kind}${context.filePath ? ` · ${context.filePath}` : ''}`
                : 'opens the provider’s own terminal with a brief; AgentDeck edits nothing'}
            </div>
          </div>
          <button type="button" onClick={onClose} aria-label="Close" className="ml-auto text-zinc-500 hover:text-zinc-200 text-xl leading-none">
            ×
          </button>
        </div>
        <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-3">
          {!done ? (
            <>
              <label className="block">
                ...
                <textarea
                  ...
                  placeholder={
                    placeholder ||
                    'Say what you want in your own words — e.g. “set this project up for me”, “format code after every edit”, “let it read our GitHub issues”. The agent asks about the details.'
                  }
              ...
              <div className="text-[11.5px] text-zinc-500 space-y-1">
                ...
                <ul className="list-disc pl-5 space-y-0.5">
                  ...
                  {context.docs && (
                    <li>
                      the{' '}
                      <a href={context.docs} target="_blank" rel="noreferrer" className="text-sky-400 hover:text-sky-300">
                        official docs page ↗
                      </a>{' '}
                      for it — read first, by instruction
                    </li>
                  )}
                  ...
                  <li>
                    how to work: read the docs, interview you first (one question at a time), search find-skills for skills your intent needs, show the plan,
                    then the diff
                  </li>
                </ul>
              </div>
              <button type="button" onClick={() => setShowBrief((s) => !s)} className="text-[11px] text-zinc-500 hover:text-zinc-200">
                {showBrief ? 'hide the brief' : 'preview the brief'}
              </button>
              {showBrief && (
                <pre className="text-[11px] leading-5 font-mono text-zinc-400 whitespace-pre-wrap break-words bg-ink-900 rounded-lg p-3 max-h-64 overflow-y-auto">
                  {brief}
                </pre>
              )}
            ...
            <div className="space-y-2 text-[13px] text-zinc-300">
              <div>
                {done.reused
                  ? `A terminal for this folder was already open — it was reused. The brief is at:`
                  : `${providerLabel} is starting in its terminal with the brief.`}
              </div>
              <div className="font-mono text-[11.5px] text-zinc-400 break-all">{done.brief}</div>
              {done.reused && (
                <div className="text-[12px] text-zinc-500">
                  Paste into that terminal: <span className="font-mono text-zinc-300">read {done.brief} and do what it asks</span>
                </div>
              )}
            </div>
          )}
        ...
        <div className="px-4 py-3 border-t border-zinc-800 shrink-0 flex items-center gap-2">
          ...
          <button type="button" onClick={onClose} className="text-[12px] px-3 py-1.5 rounded text-zinc-400 hover:text-zinc-200">
            {done ? 'Close' : 'Cancel'}
          </button>
          {!done && (
            <button
              ...
              className="text-[12px] px-3 py-1.5 rounded bg-sky-500/20 text-sky-200 hover:bg-sky-500/30 disabled:opacity-40"
            >
...
export function HandoffButton({
  onClick,
  label = 'Ask the agent',
  title = 'Hand this to the CLI: your request + the file + its official docs, in its own terminal',
  className = '',
}: {
  onClick: () => void
  label?: string
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={title}
      className={`text-[11.5px] px-2 py-1 rounded bg-violet-500/10 text-violet-200 hover:bg-violet-500/20 inline-flex items-center gap-1 ${className}`}
    >
