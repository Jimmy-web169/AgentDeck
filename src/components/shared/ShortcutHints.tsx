...
export const SHORTCUTS = [
  ...
  { keys: ['Alt', '←'], alt: ['Alt', '→'], label: 'Previous / next tab (also Alt+[ / Alt+])' },
  { keys: ['Alt', '1…9'], label: 'Jump to tab (9 = last)' },
  { keys: ['Alt', '↓'], alt: ['Alt', '↑'], label: 'Inside a tab: conversation ↔ its terminal sub-tab' },
...
export function Kbd({ children }: React.PropsWithChildren) {
  return (
    <kbd className="inline-block px-1.5 py-0.5 rounded border border-zinc-700 bg-ink-700 text-[10.5px] leading-none text-zinc-400 font-mono">{children}</kbd>
  )
}
