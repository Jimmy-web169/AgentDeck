...
export default function EarlierBar({
  startIdx,
  chunk,
  total,
  onMore,
  ...
  return (
    <div ref={topRef} className="flex items-center justify-center gap-2 py-1 text-[11px] text-zinc-500 select-none">
      <button type="button" data-earlier onClick={onMore} className="hover:text-zinc-300" title={`Show the previous ${n} — scrolling to the top does the same`}>
        ↑ {startIdx} earlier · show {n} more
      </button>
      <span className="text-zinc-700">·</span>
      <button type="button" data-earlier-all onClick={onAll} className="hover:text-zinc-300" title={`Show all ${total}`}>
        show all
      </button>
    </div>
  )
