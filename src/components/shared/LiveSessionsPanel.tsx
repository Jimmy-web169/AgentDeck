...
import type { toManagerItems } from '../../lib/useActiveSessions.ts'
type ManagerItem = ReturnType<typeof toManagerItems>[number]
interface LiveProps {
  items?: ManagerItem[]
  providers?: readonly UIProvider[]
  onEnter: (item: ManagerItem) => void
  onClose?: (item: ManagerItem) => Promise<unknown>
  onClosePanel: () => void
  title?: string
}
...
import LiveDashboards from './LiveDashboards.tsx'

// badge color comes from the provider's config (`accent`); unknown → zinc
function ProviderTag({ providers, id }: { providers?: readonly UIProvider[]; id?: string | null }) {
  if (!id) return null
...
const Row = ({
  it,
  children,
  ctx: { providers, busy, onEnter, onClose, endTerminal },
}: {
  it: ManagerItem
  children: ReactNode
  ctx: Pick<LiveProps, 'providers' | 'onEnter' | 'onClose'> & { busy: boolean; endTerminal: (it: ManagerItem) => void }
}) => (
...
export default function LiveSessionsPanel({ items = [], providers = [], onEnter, onClose, onClosePanel, title = 'Live sessions' }: LiveProps) {
  ...
  const close = () => {
    if (!busy) onClosePanel()
  }
  const endTerminal = async (it: ManagerItem) => {
  ...
  const createDashboard = useDeckMutation('createDashboard')
  const dashboard = async () => {
    setBusy(true)
    setError('')
    try {
      const d = await createDashboard.mutateAsync({ keys: selected, title: 'Live dashboard' })
      shellActions.openDeckView({ kind: 'dashboard', dashboardId: d.id, title: d.title })
      onClosePanel()
    } catch (e) {
      setError(errorMessage(e))
    } finally {
  ...
  return (
    ...
    <div
      role="presentation"
      className="fixed inset-0 z-40 flex items-start justify-center bg-black/50 pt-20"
      onClick={(e) => {
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Live sessions & dashboards"
        className="w-[640px] max-w-[92vw] max-h-[70vh] overflow-y-auto rounded-xl border border-zinc-700 bg-ink-800 shadow-2xl"
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-700">
          <div className="text-sm font-semibold text-zinc-100">{title}</div>
          <button type="button" disabled={busy} onClick={close} aria-label="Close" className="text-zinc-500 hover:text-zinc-200">
            ✕
          </button>
        </div>
        <div role="tablist" aria-label="Live manager" className="flex gap-3 px-4 pt-3 text-xs">
          {[
            ...
            <button
              ...
              disabled={busy}
              onClick={() => setView(key)}
              className={`pb-2 border-b-2 ${view === key ? 'border-sky-400 text-sky-200' : 'border-transparent text-zinc-500'}`}
            >
              {label}
            </button>
          ))}
        ...
        {view === 'dashboards' ? (
          <div className="px-4 py-3">
            <LiveDashboards onOpen={onClosePanel} onCreate={() => setView('sessions')} />
          </div>
        ) : (
          <div className="px-4 py-3">
            ...
            {items.map((it) => (
              <div key={it.key} className="flex items-start gap-2">
                <input
                  type="checkbox"
                  className="mt-3"
                  aria-label={`Include ${it.title} in dashboard`}
                  checked={selected.includes(it.key)}
                  disabled={busy || !it.tmuxName || (!selected.includes(it.key) && selected.length >= 4)}
                  onChange={(e) => setSelected((old) => (e.target.checked ? [...old, it.key] : old.filter((k) => k !== it.key)))}
                />
                <Row ctx={{ providers, busy, onEnter, onClose, endTerminal }} it={it}>
            ...
            <div className="mt-3 border-t border-zinc-700 pt-3 text-xs text-zinc-500">
              Select 2–4 existing tmux sessions. Experimental dashboard starts read-only; source agents stay where they are.
            </div>
            {error && (
            ...
            <button
              ...
              onClick={dashboard}
              className="mt-3 px-3 py-1.5 rounded bg-sky-500/20 text-sky-200 text-xs disabled:opacity-40"
            >
              {busy ? 'Creating dashboard…' : `Open dashboard (${selected.length}/4)`}
            </button>
          </div>
        )}
      </div>
