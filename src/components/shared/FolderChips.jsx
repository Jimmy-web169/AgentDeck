...
export default function FolderChips({ scopes = [], providers = [], value, onPick, onManage, compact = false }) {
  const chip = (s) => {
  ...
  return (
    <div className={compact ? 'flex items-center gap-1 overflow-x-auto no-scrollbar' : 'flex flex-wrap items-center gap-1'}>
      ...
      {onManage && (
        <button onClick={onManage} title="Track another folder / edit labels" className="shrink-0 h-6 w-6 rounded-md border border-dashed border-zinc-700 text-zinc-500 hover:text-zinc-200 hover:border-zinc-500 hover:bg-ink-700 flex items-center justify-center transition-colors">
