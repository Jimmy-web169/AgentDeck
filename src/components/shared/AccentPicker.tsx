...
import { hexToHsl, hslToHex, normalizeColor } from '../../lib/accent.ts'
import { setPref, usePrefs } from '../../lib/prefs.ts'
...
const resolve = (value: unknown, defaultValue: unknown) => normalizeColor(value) || normalizeColor(defaultValue) || '#71717a'

export function AccentTray({ value, defaultValue, onChange, onReset = null, resetLabel = 'default' }: TrayProps) {
  ...
  const pick = (nh: number, ns: number) => onChange(hslToHex(nh, ns, 55))
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: This presentation wrapper only stops outside-dismiss propagation; its child buttons own interaction.
    <div role="presentation" className="rounded-md border border-zinc-800 bg-ink-900/60 px-2.5 py-2 space-y-2" onMouseDown={(e) => e.stopPropagation()}>
        ...
        {saved.map((c) => (
          <span key={c} className="relative group/sw">
            ...
            <button
              type="button"
              onClick={() => remove(c)}
              title="Remove from your swatches"
              className="absolute -top-1.5 -right-1.5 w-3 h-3 rounded-full bg-ink-900 border border-zinc-600 text-[8px] leading-none text-zinc-300 flex items-center justify-center opacity-0 group-hover/sw:opacity-100 transition-opacity"
            >
              ×
            </button>
          </span>
      ...
      <input
        ...
        max={360}
        value={h}
        onChange={(e) => pick(Number(e.target.value), s)}
        className="accent-range w-full"
        style={{ background: HUE_TRACK, '--thumb': live }}
        title={`hue ${h}°`}
        aria-label="Hue"
      />
      <input
        ...
        max={100}
        value={s}
        onChange={(e) => pick(h, Number(e.target.value))}
        className="accent-range w-full"
        style={{ background: `linear-gradient(to right, hsl(${h} 0% 55%), hsl(${h} 100% 55%))`, '--thumb': live }}
        title={`saturation ${s}%`}
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <label
          title="System colour dialog"
          className="relative w-4 h-4 rounded-full shrink-0 cursor-pointer ring-1 ring-ink-900 opacity-80 hover:opacity-100"
        ...
        >
          <input
            ...
            aria-label="System colour dialog"
          />
        </label>
        <span className="flex-1" />
        {onReset && (
          <button type="button" onClick={onReset} title={`Back to the ${resetLabel}`} className={link}>
            {resetLabel}
          </button>
        )}
      </div>
...
export default function AccentField({
  ...
  onToggle,
  empty = 'none',
}: TrayProps & { label: React.ReactNode; hint?: React.ReactNode; open?: boolean; onToggle?: (open: boolean) => void; empty?: string }) {
  ...
  return (
    <div className="py-0.5">
      <button
        ...
        title={open ? 'Close' : 'Change colour'}
      >
        <span
          className="w-4 h-4 rounded-full shrink-0 ring-1 ring-ink-900 transition-shadow group-hover/chip:shadow-[0_0_8px_var(--glow)]"
          style={{ background: hexNow ? live : 'transparent', border: hexNow ? 'none' : '1px dashed rgb(var(--zinc-600))', '--glow': live }}
        />
