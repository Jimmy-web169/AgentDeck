import type { ActivityReport, ActivityDay } from '../../api/models.ts'
type Segment = { t: string; k?: boolean }
type LegacyWeek = Omit<ActivityReport['weekly'][number], 'projects'> & { projects: number | null }
export interface InsightsProps {
  ...
  data?: Partial<ActivityReport> | null
  embedded?: boolean
}
import { errorMessage } from '../../lib/errors.ts'
import { insightProjectKey } from '../../../shared/identity.ts'
import { useEffect, useMemo, useState } from 'react'
import { createApi } from '../../api/index.ts'
import HandoffDialog, { HandoffButton } from './HandoffDialog.tsx'
import { fmtRelative } from '../../lib/format.ts'
import { shortPath, projectName } from '../../lib/paths.ts'
...
const PART_PHRASE: Record<string, string> = { morning: 'in the morning', afternoon: 'in the afternoon', evening: 'in the evening', night: 'at night' }
const PART_PERSONA: Record<string, string> = { morning: 'Early bird', afternoon: 'Afternoon builder', evening: 'Evening coder', night: 'Night owl' }

// ---------- tiny formatting helpers ----------
const plural = (n: number, w: string) => `${n ?? 0} ${w}${n === 1 ? '' : 's'}`
const pct = (x: number) => `${Math.round((x || 0) * 100)}%`
const hh = (h: number) => `${String(h).padStart(2, '0')}:00`
const num1 = (x: number | null) => (x == null || Number.isNaN(Number(x)) ? '—' : Number(x) >= 10 ? String(Math.round(x)) : String(Math.round(x * 10) / 10))
...
const fmtDurCap = (m: number) => (m >= 12 * 60 ? `${fmtDur(m)}+` : fmtDur(m)) // durations are clamped at 12h server-side

const deltaText = (d: number) => (d > 0 ? `▲ ${d}` : d < 0 ? `▼ ${-d}` : '± 0')
...
function deriveWeekly(daily: ActivityDay[]) {
  const map = new Map<string, LegacyWeek>()
...
function deriveCompare(daily: ActivityDay[]) {
  const sum = (rows: ActivityDay[]) =>
    rows.reduce(
      (a, d) => ({ sessions: a.sessions + (d.sessions || 0), prompts: a.prompts + (d.prompts || 0), activeDays: a.activeDays + (d.sessions ? 1 : 0) }),
      { sessions: 0, prompts: 0, activeDays: 0 }
    )
  return { thisWeek: sum(daily.slice(-7)), lastWeek: sum(daily.slice(-14, -7)) }
}

function heatGrid(daily: ActivityDay[]) {
  ...
  const cells = [...Array<null>(firstDow).fill(null), ...daily]
  const weeks: (ActivityDay | null)[][] = []
  for (let i = 0; i < cells.length; i += 7) weeks.push(cells.slice(i, i + 7))
  const monthOf = (w: (ActivityDay | null)[]) => {
...
function personaOf(rhythm: ActivityReport['rhythm'] | null, totals: ActivityReport['totals']) {
  ...
  if (rhythm.part && PART_PERSONA[rhythm.part] && share >= 0.45)
    return { label: PART_PERSONA[rhythm.part], why: `${pct(share)} of sessions ${PART_PHRASE[rhythm.part]}` }
...
const T = (t: string): Segment => ({ t })
const K = (t: string): Segment => ({ t, k: true })
function sentence(clauses: Segment[][]) {
  const out: Segment[] = []
  clauses.forEach((c, i) => {
...
function narrative({
  ...
  active30: number
  sessions30: number
  totals: ActivityReport['totals']
  streak: ActivityReport['streak']
  busiest: ActivityReport['busiest']
  rhythm: ActivityReport['rhythm'] | null
  sd: ActivityReport['sessionsDetail'] | null
  ...
  if (!totals.sessions) {
    return [
      [
        T(
          `No agent sessions in the last ${HEAT_WEEKS} weeks for this folder. Once you start working here, this page fills in with your rhythm, streaks and session shape.`
        ),
      ],
    ]
  }
  ...
  if (active30 === 0) {
    out.push(
      lastActive
        ? [T(`Nothing in the last ${WINDOW} days — your last session here was on `), K(lastActive), T('.')]
        : [T(`Nothing in the last ${WINDOW} days.`)]
    )
  } else {
    const s1 = [T('You worked with agents on '), K(`${active30} of the last ${WINDOW} days`), T(` (${plural(sessions30, 'session')})`)]
    if (rhythm?.part && PART_PHRASE[rhythm.part] && rhythm.share >= 0.35)
      s1.push(T(', mostly '), K(PART_PHRASE[rhythm.part]), T(` (${pct(rhythm.share)} of sessions)`))
    s1.push(T('.'))
    out.push(s1)
  }
  ...
  if (rhythm && typeof rhythm.weekendShare === 'number') {
    ...
    clauses.push(
      ws >= 0.5
        ? [T('most of your sessions land on weekends '), T(`(${pct(ws)})`)]
        : ws < 0.1
          ? [T(`you rarely work weekends (${pct(ws)})`)]
          : [T(`${pct(ws)} of sessions land on weekends`)]
    )
  }
  if (streak.current >= 2)
    clauses.push([
      T("you're on a "),
      K(`${streak.current}-day streak`),
      T(streak.longest > streak.current ? ` (longest ${streak.longest})` : ' — your longest yet'),
    ])
  ...
  if (sd?.count) {
    ...
    if (bits.length) {
      ...
      if (sd.longest?.minutes != null)
        s3.push(
          T('; the longest was '),
          K(fmtDurCap(sd.longest.minutes)),
          T(' in '),
          K(shortPath(sd.longest.cwd || sd.longest.slug)),
          T(sd.longest.date ? ` on ${sd.longest.date}` : '')
        )
      s3.push(T('.'))
      out.push(s3)
    }
...
function derive(data: Partial<ActivityReport>) {
  const daily = Array.isArray(data.daily) ? data.daily : []
  const last30 = daily.slice(-WINDOW)
  const active30 = last30.filter((d: { sessions: number }) => d.sessions > 0).length
  ...
  const hours = Array.isArray(data.hours) && data.hours.length === 24 ? data.hours : Array<number>(24).fill(0)
  const weekdays = Array.isArray(data.weekdays) && data.weekdays.length === 7 ? data.weekdays : Array<number>(7).fill(0)
...
function digestMd(
  v: ReturnType<typeof derive>,
  { providerLabel, rootLabel, range }: { providerLabel: string; rootLabel: string; range?: ActivityReport['range'] }
) {
  const flat = (segs: Segment[]) => segs.map((s) => (s.k ? `**${s.t}**` : s.t)).join('')
  ...
  const lines = [
    ...
    v.busiest.hour != null ? `- Peaks: ${hh(v.busiest.hour)} · ${WEEKDAYS_LONG[v.busiest.weekday ?? -1] || '—'}s` : '- Peaks: —',
    '',
    '## Weekly',
    ...
    ...(v.neglected?.length
      ? [
          '',
          '## Needs attention',
          ...v.neglected.map((p) => `- ${shortPath(p.cwd || p.slug)} — idle ${plural(p.daysAgo, 'day')} · ${plural(p.sessions, 'session')}`),
        ]
      : []),
    '',
...
function Card({ title, right, children, className = '' }: React.PropsWithChildren<{ title: React.ReactNode; right?: React.ReactNode; className?: string }>) {
  return (
...
function Ring({ ratio, size = 40, stroke = 4 }: { ratio: number; size?: number; stroke?: number }) {
  ...
  return (
    <div>
      <div className="flex items-end gap-[2px] border-b border-zinc-800/70" style={{ height }}>
        {values.map((v, i) => (
          // biome-ignore lint/suspicious/noArrayIndexKey: These presentation-only chart bins and generated sentence segments have fixed ordinal positions, including repeated labels.
          <div key={i} className="flex-1 min-w-0 h-full flex flex-col justify-end" title={titles[i]}>
            ...
            <div
              className={`w-full rounded-t-[4px] transition-colors ${v ? (peak === i ? 'bg-emerald-500/95' : 'bg-emerald-500/60 hover:bg-emerald-500/85') : 'bg-ink-700'}`}
              style={{ height: v ? Math.max(3, Math.round((v / max) * (height - top))) : 2 }}
            />
          </div>
        ))}
      </div>
...
function HBars({ items, unit = 'session' }: { items: { label: string; n: number }[]; unit?: string }) {
...
function Heatmap({ grid }: { grid: ReturnType<typeof heatGrid> }) {
            ...
            {weeks.map((w, wi) => (
              ...
              <div key={wi} className="flex flex-col gap-[3px]">
                {Array.from({ length: 7 }, (_, di) => {
                  ...
                  return d ? (
                    <div
                      // biome-ignore lint/suspicious/noArrayIndexKey: Seven fixed weekday cells retain their positions, including empty edge days.
                      key={di}
                      className={`w-3 h-3 rounded-[3px] ${heat(d.sessions, max)}`}
                      title={`${d.date} · ${plural(d.sessions, 'session')} · ${plural(d.prompts, 'prompt')} · ${plural(d.toolCalls, 'tool call')}`}
                    />
                  ) : (
                    // biome-ignore lint/suspicious/noArrayIndexKey: Empty cells share the same fixed seven-day grid positions.
...
// ---------- page ----------
export default function InsightsPage({ provider, root, rootLabel = '', providerLabel = '', onOpen, data: suppliedData, embedded = false }: InsightsProps) {
  usePrefs() // re-render when Preferences › Paths changes (shortPath reads it)
  const [fetchedData, setData] = useState<ActivityReport | null>(null)
  ...
  useEffect(() => {
    ...
    setErr(null)
    createApi(provider)
      .activity({ root, days: HEAT_WEEKS * 7 })
  ...
  if (!v || !data) return <div className="p-8 text-zinc-600 text-sm">Reading sessions…</div>
  ...
  const weekTitles = weekly.map(
    (w) =>
      `Week of ${w.weekStart} · ${plural(w.sessions, 'session')} · ${plural(w.prompts, 'prompt')} · ${plural(w.activeDays, 'active day')}${w.projects != null ? ` · ${plural(w.projects, 'project')}` : ''}`
  )
    ...
    <div className={`min-w-0 mx-auto max-w-6xl space-y-5 ${embedded ? '' : 'px-6 py-6'}`}>
      ...
      <section className="rounded-xl border border-zinc-800 bg-ink-900 p-5">
        <div className="flex flex-wrap items-start gap-3">
          ...
          {v.persona && (
            <span
              className="inline-flex items-center gap-1.5 rounded-full border border-zinc-700 bg-ink-800 px-2.5 py-1 text-[11.5px] text-zinc-200 shrink-0"
              title={v.persona.why}
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3">
        <Tile
          ...
          value={v.active30}
          unit={`/ ${WINDOW}`}
          sub={v.totals.activeDays != null ? `${v.totals.activeDays} in ${HEAT_WEEKS} weeks` : undefined}
          aside={<Ring ratio={v.active30 / WINDOW} />}
        />
        ...
        <Tile
          ...
          aside={
            <span
              className={`text-[13px] font-medium tabular-nums shrink-0 ${delta > 0 ? 'text-zinc-200' : 'text-zinc-500'}`}
              title={`${cmp.thisWeek.sessions} this week vs ${cmp.lastWeek.sessions} the week before`}
            >
              {deltaText(delta)}
            </span>
          }
        ...
        <Tile
          ...
          value={sd?.medianPrompts != null ? String(sd.medianPrompts) : v.pps}
          sub={sd?.medianPrompts != null ? `average ${v.pps}` : sd ? undefined : 'average over the window'}
        />
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[auto_1fr_1fr]">
        ...
        <Card
          ...
          right={
            <span className="text-[11px] text-zinc-500">
              {hourPeak != null ? `peaks at ${hh(hourPeak)} · ${plural(v.hours[hourPeak], 'session')}` : 'no sessions yet'} · local time
            </span>
          }
        >
          <Bars
            values={v.hours}
            labels={v.hours.map((_, h) => `${h}`)}
            titles={v.hours.map((n, h) => `${hh(h)} · ${plural(n, 'session')}`)}
            every={3}
            height={88}
            peak={hourPeak}
            showValues
        ...
        <Card
          ...
          right={
            <span className="text-[11px] text-zinc-500">
              {dayPeak != null ? `peaks on ${WEEKDAYS_LONG[dayPeak]}s · ${plural(v.weekdays[dayPeak], 'session')}` : 'no sessions yet'}
            </span>
          }
        >
          <Bars
            values={MON_FIRST.map((i) => v.weekdays[i] || 0)}
            labels={MON_FIRST.map((i) => WEEKDAYS[i])}
            titles={MON_FIRST.map((i) => `${WEEKDAYS_LONG[i]} · ${plural(v.weekdays[i] || 0, 'session')}`)}
            height={88}
            peak={dayPeak != null ? MON_FIRST.indexOf(dayPeak) : null}
            showValues
          />
        </Card>
      </div>
      ...
      <Card
        ...
        right={
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => setTable((t) => !t)} className={toggleBtn}>
              {table ? 'Chart' : 'Table'}
            </button>
          </div>
        }
      >
        {weekly.length === 0 ? (
          ...
          <Bars
            values={weekly.map((w) => w.sessions || 0)}
            labels={weekly.map((w) => shortDate(w.weekStart))}
            subLabels={weekly.map((w) => `${w.activeDays}d`)}
            titles={weekTitles}
            showValues
            height={130}
          />
        ...
        <Card title="Session length" right={sd?.count != null ? <span className="text-[11px] text-zinc-500">{plural(sd.count, 'session')}</span> : null}>
          {sd ? (
            <>
              ...
              {sd.longest && (
                <div
                  className="mt-3 pt-3 border-t border-zinc-800/70 text-[12px] text-zinc-500 truncate"
                  title={`${sd.longest.title || ''}${sd.longest.cwd ? ` · ${sd.longest.cwd}` : ''}`}
                >
      ...
      <Card title="Needs attention" right={<span className="text-[11px] text-zinc-500">projects you touched this window but not in the last 14 days</span>}>
        {v.neglected == null ? (
          ...
          <div className="text-[12.5px] text-zinc-500">
            Nothing is gathering dust — every project you touched in the last {HEAT_WEEKS} weeks has seen a session within two weeks.
          </div>
        ) : (
          <div className="-mx-2">
            {v.neglected.map((p) => {
              ...
              return onOpen ? (
                <button
                  type="button"
                  key={insightProjectKey(p.slug || '', p.cwd || '')}
                  onClick={() => onOpen({ root, slug: p.slug, cwd: p.cwd, project: projectName(p.cwd, p.slug) })}
                  className={`${cls} hover:bg-ink-800`}
                  title={p.cwd || p.slug || undefined}
                >
                  {inner}
                </button>
              ) : (
                <div key={insightProjectKey(p.slug || '', p.cwd || '')} className={cls} title={p.cwd || p.slug || undefined}>
                  {inner}
                </div>
              )
            })}
      ...
      <div className="flex flex-wrap items-center gap-3 pt-1 text-[11.5px] text-zinc-500">
        <button type="button" onClick={copy} className="px-3 py-1.5 rounded-md bg-ink-700 border border-zinc-700 text-[12px] text-zinc-200 hover:bg-ink-600">
          {copied ? '✓ copied' : 'Copy digest as Markdown'}
        </button>
        {provider && root && (
          <HandoffButton
            onClick={() => setHandoff(true)}
            label={`Ask ${providerLabel || 'the agent'} about it`}
            title="Open the CLI with this digest and a question about your habits"
          />
        )}
        <span>
          {provider && root ? 'Paste it into any agent session, or hand it over directly.' : 'Copy the combined digest into the agent session of your choice.'}
        </span>
      ...
      {handoff && provider && root && (
        <HandoffDialog
          ...
          root={root}
          context={{
            kind: 'Insights digest (last 30 days)',
            filePath: null,
            content: digestMd(v, { providerLabel, rootLabel, range: data?.range }),
            docs: null,
            need: 'Read the digest below and tell me the three habits worth changing, with one concrete experiment for each. Do not repeat the numbers back to me.',
          }}
