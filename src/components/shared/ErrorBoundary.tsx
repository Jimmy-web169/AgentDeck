type Props = React.PropsWithChildren<{ resetKey?: unknown; label?: string }>
...
export default class ErrorBoundary extends Component<Props, { hasError: boolean; error: unknown }> {
    ...
    return (
      <div className="m-4 rounded-lg border border-red-500/40 bg-red-500/10 p-4 text-[13px]">
        ...
        <pre className="text-[11.5px] text-red-200/80 font-mono whitespace-pre-wrap break-words max-h-40 overflow-y-auto">
          {String(
            (this.state.error && typeof this.state.error === 'object' && 'message' in this.state.error ? this.state.error.message : null) || this.state.error
          )}
        </pre>
