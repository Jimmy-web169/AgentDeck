...
export function fromHash(hash: unknown, knownProviders: string[] = []): import('./types.d.ts').Target | null {
  const h = String(hash || '')
  if (!h.startsWith('#/')) return null
  const [route, search = ''] = h.split('?')
  const segs = route.slice(2).split('/').filter(Boolean).map(dec)
  if (!segs.length || segs[0] === 'home') {
    const target: import('./types.d.ts').Target = { provider: null, ...(segs.length ? { view: segs[1] || 'activity' } : {}) }
    const query = new URLSearchParams(search)
    try {
      if (query.has('scope')) target.homeScope = normalizeHomeScope(JSON.parse(query.get('scope') ?? 'null'))
    } catch {}
    try {
      const source = normalizeHomeSource(JSON.parse(query.get('source') ?? 'null'))
      if (source) target.homeSource = source
    } catch {}
    return target
  }
  ...
  const t: import('./types.d.ts').Target = { provider }
  if (root) t.root = root
  if (slug) t.slug = slug
  if (id) t.id = id
  const query = new URLSearchParams(search)
